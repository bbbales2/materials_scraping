...
class LoaderStopwords(object):
    ...
    def __iter__(self):
        ...
        for sentence in itertools.chain(*[session.query(cls).order_by(cls.id).yield_per(100) for cls in self.clss]):
...
class Word2VecStopsLoader(object):
    ...
    def __iter__(self):
        notword = re.compile(r'[^\w -]')
        for sentence in itertools.chain(*[session.query(cls).yield_per(100) for cls in self.clss]):
...
word2vec.save('/home/bbales2/scraping/webpage/db/{0}.word2vec.big'.format(prefix))
...
sys.stdout.flush()
dictionary.save('/home/bbales2/scraping/webpage/db/{0}.dictionary.big'.format(prefix))
...
sys.stdout.flush()
tfidf.save('/home/bbales2/scraping/webpage/db/{0}.tfidf.big'.format(prefix))
...
lsi.save('/home/bbales2/scraping/webpage/db/{0}.lsi.big'.format(prefix))
sys.stdout.flush()

tmp = time.time()
index = gensim.similarities.docsim.Similarity('/home/bbales2/scraping/lsi_index/{0}.index.big'.format(prefix), LSILoader([mldb2.BodySentence, mldb2.AbstractSentence, mldb2.FigureSentence]), num_features = numberTopics)
print 'Index Time: ', time.time() - tmp
sys.stdout.flush()
index.save('/home/bbales2/scraping/webpage/db/{0}.index.big'.format(prefix))